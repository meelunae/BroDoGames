		...
		if(c != null && c.getProdotti().size() != 0){
			...
			<tr>
				...
				<td>
					...
					<input type=number value="<%=i.getQtaFis() %>" name=newQtaFis>
					<input type=number value="<%=i.getQtaDig() %>" name=newQtaDig>
					<input type="hidden" name="action" value="updateQta">
					<input type="hidden" name="id" value=<%=i.getProdotto().getId() %>>
					<input type="submit">
				</form>
				</td>
